...
export function PageFeedbackToolbarCSS({
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      const textTags = new Set(["P", "SPAN", "H1", "H2", "H3", "H4", "H5", "H6", "LI", "TD", "TH", "LABEL",
        "BLOCKQUOTE", "FIGCAPTION", "CAPTION", "LEGEND", "DT", "DD", "PRE", "CODE", "EM", "STRONG",
        "B", "I", "U", "S", "A", "TIME", "ADDRESS", "CITE", "Q", "ABBR", "DFN", "MARK", "SMALL", "SUB", "SUP"]);
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      if (isDragging && dragStart) {
        ...
        const finalElements = allMatching.filter(({ element: el }) =>
          !allMatching.some(({ element: other }) => other !== el && el.contains(other))
        );
  ...
  return createPortal(
    <>
      ...
      <div className={styles.toolbar} data-feedback-toolbar>
        ...
        {isActiveVisible && (
          <div className={`${styles.controls} ${isActive ? styles.enter : styles.exit}`}>
            <button
        ...
        {showSettingsVisible && (
          ...
          >
            <div className={styles.settingsHeader}>
              <svg width="80" height="14" viewBox="0 0 124 22" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Agentation">
            ...
            <div className={styles.settingsSection}>
              ...
              <div className={styles.settingsOptions}>
                {OUTPUT_DETAIL_OPTIONS.map((option) => (
                  ...
                  >
                    {settings.outputDetail === option.value && <IconCheck size={10} />}
                    {option.label}
                  </button>
                ))}
              </div>
      ...
      <div className={styles.markersLayer} data-feedback-toolbar>
        {markersVisible &&
          visibleAnnotations.filter(a => !a.isFixed).map((annotation, index) => {
        ...
        {markersVisible && !markersExiting &&
          exitingAnnotationsList.filter(a => !a.isFixed).map((annotation) => {
            const isMulti = annotation.isMultiSelect;
            return (
              <div
                key={annotation.id}
